...
<script type="text/javascript">
		...
		$.each(dataSourceListObject,function(index,indexEntry){
			...
			//if($(indexEntry).val()!=0){
				...
				if(index==0){
					...
					/*
					dataSourceList+="{";
					dataSourceList+="\"id\":\""+$(indexEntry).val()+"\",";
					dataSourceList+="\"name\":\""+$(indexEntry).text()+"\"";
					dataSourceList+="}";
					*/
				}else{
					dataSourceListArray+=",\""+$(indexEntry).val()+"-"+$(indexEntry).text()+"\"";
	...
	$(document).ready(function(){
		...
		//alert("hello jqeury");
		console.log(getObjectDatasourceListFn());
		
	

	    		$("#datasourceId").autocomplete({
...
<body>
	...
	<div class='empbedParamChartDataSource'>
	</div>
	<form:form id="chartDatasource" modelAttribute="chartDatasourceForm" method="post" name="chartDatasourceForm" action="${formAction}" enctype="multipart/form-data">
		...
		<div class="chart_ds_name">
			...
			<form:select style="min-width:70%; display:'';" id="datasourceId"  class='inputFormChartDataSource' path="datasourceId" onchange="setDatasource(this)">
